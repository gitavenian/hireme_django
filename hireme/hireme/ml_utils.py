...
def check_job_suitability(job_announcement, user):
    main_skill = job_announcement.main_skill
    ...
    if main_skill in user_skill_names:
        user_experience = user_skill_experiences[main_skill]
        feature_vector = [
            main_skill,
            user_experience,
            main_skill,
        ...
        highest_skill = max(user_skill_experiences, key=user_skill_experiences.get, default=None)
        highest_experience = user_skill_experiences.get(highest_skill, 0)
        feature_vector = [
            highest_skill,
            0,
            main_skill,
    ...
    encoded_features = np.array(encoded_features).reshape(1, -1)
    job_suitability = convenient_job_model.predict(encoded_features)[0]
